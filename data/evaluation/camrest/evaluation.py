...
class Retrieve_Metric(object):
    def __init__(self, retrieve_score, data, db, GTKB=False):
        self.GTKB = GTKB
    ...
    def calc_recall(self, level="turn_level", top_k=7, first_turn_name=True):
        if self.GTKB:
            top_k_index = self.retrieve_score
        else:
            top_k_index = self.retrieve_score.sort(-1, True)[1][:,:top_k].tolist()
...
class Metric_data0_new1(object):
    """
    BLEU:
    F1:
    """
    ...
    def __init__(self, data):
        ...
        self.entities_path = "../../data/RRG_cdnet_data0_times_gtdb_gesa_times-cr/entities.json"
    ...
    def clean_gen_sentence(self, text):
        text = text.replace("<sys> ", "").replace(" </s>", ""). \
            replace("<user>", "").replace("<database>", "").replace("<sep_attributes>", "").strip()
        return text
