...
from transformers.models.bert.modeling_bert import BertPooler, BertEmbeddings, BertEncoder, BertLMPredictionHead
...
def cl_init(cls, config, pooler_type, temp, pooler_num):
    ...
    # TODO: 真正的池化方式应该是改上一行，参数里的池化方式只是为了加MLP
    if pooler_type == "cls":
...
def sentemb_forward(
    ...
    # TODO:这里一定是return dict才可以过cls.pooler
    outputs = encoder(
        input_ids,
        attention_mask=attention_mask,
        token_type_ids=token_type_ids,
...
class BertForCL(BertPreTrainedModel):
    ...
    def __init__(self, config, pooler_type='cls', temp=0.05, pooler_num=128, do_mlm=False, mlm_weight=None,
        ...
        # # TODO: 真正的池化方式应该是改上一行，参数里的池化方式只是为了加MLP
        # #if pooler_type == "cls":
        # self.mlp = MLPLayer(config, pooler_num=pooler_num)
